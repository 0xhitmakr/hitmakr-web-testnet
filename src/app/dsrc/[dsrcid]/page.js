...
export default function DSRCAnalytics(props) {
  const { dsrcid } = props.params;
  ...
  const { earnings, loading: earningsLoading } =
    useGetEarningsInfo(dsrcAddress);

  useEffect(() => {
    const fetchSocialStats = async () => {
      try {
        const likesRes = await fetch(
          `${API_BASE_URL}/heart/dsrc/${dsrcid}/likes`
        );
        const commentsRes = await fetch(
          `${API_BASE_URL}/comment/dsrc/${dsrcid}/comments?page=1&limit=1`
        );
  ...
  // Debugging logs - Inspect details and earnings
  useEffect(() => {
    console.log("DSRCAnalytics - DSRC Details:", details);
    console.log("DSRCAnalytics - Earnings Info:", earnings);
  }, [details, earnings]);
  ...
  return (
    <section className={styles.wrapper}>
            ...
            <div className={styles.royaltySplitsCard}>
              ...
              <div className={styles.splitsContainer}>
                {details.royaltySplits.map((split, index) => {
                  // Debugging: Log each split object
                  console.log(`DSRCAnalytics - Royalty Split ${index}:`, split);
                  return (
                    <div key={index} className={styles.splitItem}>
                      <span className={styles.splitRecipient}>
                        {split.recipient ? `${split.recipient.slice(
                          0,
                          6
                        )}...${split.recipient.slice(-4)}` : 'N/A Recipient'}
                      </span>
                      <span className={styles.splitPercentage}>
                        {split.percentage != null ? String((split.percentage / 100).toFixed(0)) + '%' : 'N/A %'} {/* Ensure percentage is a String */}
                      </span>
                    </div>
                  );
                })}
      ...
      <PurchaseCard data={{ ...props?.data, royalty: details.royaltySplits }} />
    </section>
  );
}