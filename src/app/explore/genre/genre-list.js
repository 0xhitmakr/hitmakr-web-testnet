...
import secondStyles from "./genre.module.css";
const API_BASE_URL = process.env.NEXT_PUBLIC_HITMAKR_SERVER;

export default function GenreList({ searchParams }) {
    ...
    const fetchTopLiked = async () => {
      try {
        ...
        const response = await fetch(
          `${API_BASE_URL}/song/genre/${
            searchParams?.genre || "Techno"
          }?country=${searchParams?.location || ""}&date=${searchParams.date || ""}&likes=${searchParams.likes || ""}`
        );
