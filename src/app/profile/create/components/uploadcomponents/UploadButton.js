...
export default function UploadButton() {
  ...
  const resetUploadState = useResetRecoilState(
    HitmakrCreativesStore.CreativesUpload
  );
  const resetNewCreativeUpload = useResetRecoilState(
    HitmakrCreativesStore.NewCreativeUpload
  );
  const setNewCreativeUpload = useSetRecoilState(
    HitmakrCreativesStore.NewCreativeUpload
  );
  ...
  const prepareRoyaltySplits = useCallback(() => {
    ...
    for (const split of uploadState.royaltySplits) {
      ...
      if (!ethers.isAddress(normalizedAddress)) {
        throw new Error(
          `Invalid address format for ${split.role}: ${split.address}`
        );
      }
      ...
      if (isNaN(basisPoints) || basisPoints < 0 || basisPoints > BASIS_POINTS) {
        throw new Error(
          `Invalid percentage for ${split.role}: ${split.percentage}`
        );
      }
    ...
    if (total !== BASIS_POINTS) {
      throw new Error(
        `Total percentage must equal exactly 100% (got ${total / 100}%)`
      );
    }
  ...
  const addToCollection = async (dsrcId, collectionId) => {
    if (!address || !dsrcId || !collectionId) {
      console.log("Missing required information to add to collection.");
      return false;
    }
    ...
    try {
      ...
      const addResponse = await fetch(
        `${API_BASE_URL}/collection/collections/${collectionId}/tracks`,
        {
          method: "POST",
          headers: {
  ...
  const handleDSRCCreation = async (tokenURI, uploadHash) => {
    ...
    if (typeof currentYearCount !== "number")
      throw new Error("Invalid year count");
    ...
    try {
      ...
      const dsrcParams = {
        tokenURI,
        collectorsPrice: ethers
          .parseUnits(uploadState.editions.collectors.price.toString(), 6)
          .toString(),
        licensingPrice: ethers
          .parseUnits(uploadState.editions.licensing.price.toString(), 6)
          .toString(),
        recipients,
        percentages,
  ...
  const handleUpload = async () => {
    ...
    formData.append(
      "songDetails",
      JSON.stringify(uploadState.songDetails || {})
    );
    ...
    formData.append(
      "royaltySplits",
      JSON.stringify(uploadState.royaltySplits || [])
    );
    ...
    formData.append(
      "selectedChain",
      uploadState.selectedChain || DEFAULT_CHAIN
    );
    formData.append(
      "supportedChains",
      JSON.stringify([uploadState.selectedChain || DEFAULT_CHAIN])
    );

    if (
      uploadState.selectedCategory?.toLowerCase() === "music" &&
      uploadState.selectedLyrics?.trim()
    ) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const dsrcResult = await handleDSRCCreation(tokenURI, uploadHash);
      toast.dismiss();

      // Add to collection if selected
      if (uploadState.selectedCollectionId) {
        toast.loading("Adding to collection...");
        const dsrcId = dsrcResult.data.dsrcId;
        console.log("dsrc id ", dsrcResult);
        if (dsrcId) {
          const addedToCollection = await addToCollection(
            dsrcId,
            uploadState.selectedCollectionId
          );
          if (addedToCollection) {
            toast.success("Added to collection successfully!");
          } else {
            toast.error(
              "Failed to add to collection, but DSRC was created successfully."
            );
          }
        } else {
          toast.error("Could not add to collection: DSRC ID not found");
        }
        toast.dismiss();
      }
