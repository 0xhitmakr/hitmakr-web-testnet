    ...
    const fetchTopCommented = async () => {
      try {
        const response = await fetch(
          `${API_BASE_URL}/comment/top-commented?days=7&limit=10`
        );
        if (!response.ok) throw new Error("Failed to fetch top DSRCs");
        const data = await response.json();
        setTopDSRCIds(data.results);
      } catch (error) {
      ...
      <div className={styles.scrollContainer}>
        {topDSRCIds.map((song, idx) => (
          <DSRCCard
            ...
            showStats={true}
            stats={renderCommentStats(song.dsrcId)}
            hashTags={song.songDetails?.hashTags}
            songId={song?.songDetails?._id}
          />
        ))}
      </div>
